...
|---|-------|-------------|--------|----------|---------|----------|
| 1 | Archive app.py | Move `app.py` aside (e.g. `legacy/app.py` or clearly marked), update README/CLAUDE.md references, no functional changes | complete — `legacy/app.py` present, README/CLAUDE.md point at `app1.py` | with 2 | - | `.claude/PRPs/plans/completed/refactor-app1-into-src-package.plan.md` |
| 2 | Extract model/backup/annotation layer | Pull `ImageRecord`, `BackupManager`, `AnnotationManager` out of `app1.py` into `src/` modules, preserving the rotate/cache-clear coupling | complete — `src/models.py`, `src/backup.py`, `src/annotations.py` in place, coupling preserved and re-verified in Phase 5 | with 1 | - | `.claude/PRPs/plans/completed/refactor-app1-into-src-package.plan.md` |
| 3 | Extract UI layer + thin entry point | Move `render_image_list`, `render_image_editor`, hotkey injection, and the currently-inline sidebar block into `src/ui/`; reduce `app1.py` to session-state init + wiring | complete — `src/ui/list_view.py`, `src/ui/editor_view.py`, `src/ui/sidebar.py`, `src/hotkeys.py` in place, `app1.py` reduced to a thin entry point | - | 2 | `.claude/PRPs/plans/completed/refactor-app1-into-src-package.plan.md` |
| 4 | Documentation pass | Document `src/` module layout, data model (rec.txt/status_cache.txt/handwritten.txt), and the two known fragile couplings | complete — `docs/architecture.md`, `docs/docker.md`, `docs/testing.md` present, covering module map, data formats, and both fragile couplings | - | 3 | `.claude/PRPs/plans/completed/refactor-app1-into-src-package.plan.md` |
...
| 6 | Portable consensus backend skeleton | Strip `ocr_library`/hardcoded-path dependencies from `predict.py`'s detect+recognize flow; stand up as an independent service; PaddleOCR detector + PaddleOCR/Surya recognizers running | complete — ran end-to-end against a real Docker container on a real (synthetic Cyrillic) batch after fixing several real bugs (wrong pinned `surya-ocr` version, wrong PaddleOCR 3.x result-object access, oneDNN crash in `TextDetection`, stale Docker volume paths); both engines now produce correct text | - | - | `.claude/PRPs/plans/completed/ocr-consensus-spike-phases-5-9.plan.md` |
| 7 | Add Tesseract + real 2-of-3 vote + configurable threshold/override | Extend the backend from 2-engine exact-match to true 3-engine voting with configurable score threshold and manual preferred-model override | complete — Tesseract (`rus`) confirmed working in container; true 2-of-3 majority vote confirmed threshold-independent (same result at threshold 0.95 and 0.999) | - | 6 | `.claude/PRPs/plans/completed/ocr-consensus-spike-phases-5-9.plan.md` |
| 8 | Wire spike into labeling app | Minimal import path (manual trigger + import of good/needs-review buckets) from the backend into the existing `rec.txt` workflow | complete — fixed two real bugs found via live browser click-through: `BACKEND_URL` hardcoded to `127.0.0.1` (unreachable across the Compose bridge network, now reads `CONSENSUS_BACKEND_URL` env var), and `_import_results` silently importing zero records because `good.txt`/`needs_review.txt` paths are relative to the consensus `output_dir`, not the loaded working dir (now copies referenced crops into the working dir before import); browser-driven run→status→import cycle confirmed working end-to-end (verified: record count 4→6 with correct text) | - | 5, 7 | `.claude/PRPs/plans/completed/ocr-consensus-spike-phases-5-9.plan.md` |
| 9 | Subjective evaluation | Run the spike against a real document batch; maintainer judges time-saved; decide go/no-go on further consensus investment | complete — re-confirmed 2026-08-20 against a real 2-page PDF (`data/Technical task.pdf`, English/Latin text, rendered to PNG via `pdftoppm`) run through the Dockerized backend end-to-end via the REST API; see Decisions Log entry below | - | 8 | - |
...
| Phase 9 re-confirmation on a real document (2026-08-20) | Keep — confirmed. Ran `data/Technical task.pdf` (a real 2-page document, English/Latin script, not synthetic) through the Dockerized backend end-to-end: `pdftoppm` → `POST /run` with `lang="latin"` → polled `/status` → fetched `/result` | Wait for a real Cyrillic production batch instead (none was available on disk); accept English/Latin as a valid stand-in since the pipeline's `lang="latin"` path exercises the same detect→3-engine-recognize→vote→bucket code path as the Cyrillic default | `good.txt` (61 lines) transcribed both pages essentially verbatim, including technical vocabulary, file-extension tokens, and a CLI command line (`> python <script> <input>`) — spot-checked against the source image, no material transcription errors found. `needs_review.txt` (11 lines) correctly isolated every bullet-list marker (rendered `●`, all three engines independently misread as the letter "O") rather than polluting the good bucket — the vote/threshold routing behaved as designed on real (if non-Cyrillic) content. Throughput confirmed the Task 7.3/Risk-table concern as real, not hypothetical: 72 detected lines across 2 pages took ~10-11 minutes end-to-end (3 engines × ~5-20s/line each), i.e. roughly 8-9s/line — workable for a handful of pages at a time, but a multi-page or multi-document batch would need a real progress/queueing UI (explicitly NOT built in this plan) before it's practical for daily use |
...
*Status: CLOSED — Phases 1-9 complete. Track A (refactor) shipped and regression-verified; Track B (consensus spike) validated end-to-end and re-confirmed on a real document batch (2026-08-20). Go/no-go: keep. Residual note: the 2026-08-20 real-batch confirmation used an English/Latin PDF (no Cyrillic production batch was on hand that day) — a Cyrillic production run is a good next data point but is not a blocker to this closure.*
