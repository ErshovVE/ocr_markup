...
def render_image_list(manager: AnnotationManager, filtered_images: List[str]):
    ...
    with st.container(height=350):
        for i, img_name in enumerate(filtered_images[start:end]):
            ...
            if global_idx == st.session_state.current_idx:
                ...
                if st.button(display, key=f"img_{global_idx}", use_container_width=True):
    ...
    with col1:
        if st.button("◀", disabled=st.session_state.current_page == 0, key="prev_page_btn", use_container_width=True):
            st.session_state.current_page -= 1
            st.rerun()
    with col2:
        st.markdown(f"<center>{st.session_state.current_page + 1} / {total_pages}</center>", unsafe_allow_html=True)
    with col3:
        if st.button("▶", disabled=st.session_state.current_page == total_pages - 1, key="next_page_btn", use_container_width=True):
            st.session_state.current_page += 1
            st.rerun()
...
def render_image_editor(manager: AnnotationManager):
    ...
    if st.session_state.confirm_delete == current_name:
        ...
        with col_confirm1:
            if st.button("✓ Да", type="primary", key=f"delete_confirm_{current_name}", use_container_width=True):
                if manager.delete_record(current_name, create_backup=True):
                    ...
                    st.rerun()
        with col_confirm2:
            if st.button("✗ Отмена", key=f"delete_cancel_{current_name}", use_container_width=True):
                st.session_state.confirm_delete = None
                st.rerun()
        return
    ...
    with st.form(key=f"form_{current_name}"):
        ...
        with col1:
            submit = st.form_submit_button("✓ Подтвердить", use_container_width=True, type="primary")
        with col2:
            handwritten = st.form_submit_button("✍ Рукописный", use_container_width=True)
    ...
    with col1:
        if st.button("🗑️", use_container_width=True, key=f"delete_btn_{current_name}", help="Удалить"):
            st.session_state.confirm_delete = current_name
            st.rerun()
    
    with col2:
        if st.button("↶", use_container_width=True, key=f"rotate_left_{current_name}", help="Повернуть влево"):
            if rotate_image(record.absolute_path, "left"):
                st.rerun()
    
    with col3:
        if st.button("↷", use_container_width=True, key=f"rotate_right_{current_name}", help="Повернуть вправо"):
            if rotate_image(record.absolute_path, "right"):
                st.rerun()
    
    with col4:
        prev_disabled = st.session_state.current_idx == 0
        if st.button("←", use_container_width=True, disabled=prev_disabled, key=f"prev_{current_name}", help="Предыдущее изображение"):
            st.session_state.current_idx -= 1
            st.rerun()
    
    with col5:
        next_disabled = st.session_state.current_idx >= len(img_names) - 1
        if st.button("→", use_container_width=True, disabled=next_disabled, key=f"next_{current_name}", help="Следующее изображение"):
            st.session_state.current_idx += 1
            st.rerun()
    ...
    if st.session_state.unsaved_changes > 0:
        ...
        with col_save:
            if st.button("💾 Сохранить", type="primary", key=f"save_now_{current_name}", use_container_width=True):
                success, msg = manager.save_changes()
                if success:
                    st.session_state.unsaved_changes = 0
                    st.success("✓")
...
def main():
    ...
    if uploaded_file:
        ...
        if st.sidebar.button("💾 Сохранить всё", use_container_width=True, type="primary", key="sidebar_save_all"):
            success, msg = manager.save_changes()
            if success:
                st.session_state.unsaved_changes = 0
        ...
        if backups:
            ...
            if st.sidebar.button("📋", use_container_width=True, key="show_backups_btn", help="Показать/скрыть"):
